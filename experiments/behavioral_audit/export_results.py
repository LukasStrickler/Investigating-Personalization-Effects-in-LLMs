...
CSVs of one run-tag into
    ...
    experiments/behavioral_audit/results_<run-tag>/
...
RESULTS_PARENT = REPO_ROOT / "experiments" / "behavioral_audit"
...
def _copy_stage1(experiment_name: str, dest: Path, alias: str | None) -> list[dict]:
    ...
    for q in ("q1", "q2"):
        ...
        print(f"  [{q}] stage1: {len(files)} files -> "
              f"{ddir.relative_to(REPO_ROOT)}  ({succ}/{rows} {alias or 'cells'} success)")
        exported.append({"question": q, "kind": "stage1", "files": files,
                         "success": succ, "rows": rows})
    return exported
    ...
    for pat in patterns:
        for match in sorted(search_dir.glob(pat)):
            ...
            if match.is_dir():
                ...
                print(f"  {label}: dir {match.name}/ ({n} files) -> {target.relative_to(REPO_ROOT)}")
...
def main() -> None:
    ...
    ap.add_argument("--subject-alias", default=None,
                    help="model column to report success counts for (e.g. gemma-4-e2b_modal)")
    ap.add_argument("--dest", type=Path, default=None,
                    help="override destination (default experiments/behavioral_audit/results_<tag>)")
    args = ap.parse_args()
    ...
    manifest: dict = {"run_tag": args.run_tag, "subject_alias": args.subject_alias,
                      "experiment_name": experiment_name, "exported": []}
    ...
    if n_stage2 == 0:
        print("  (Stage-2 judgments not found yet — run run_behavioral_audit.py "
