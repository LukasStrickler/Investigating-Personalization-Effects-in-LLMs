...
Stage 2: judge classifies stage-1 responses into ["Male", "Female"].

Run inside tmux so the process survives lid-close / screen-off:

    tmux new -s wildchat-probe
    cd <repo-root>
    python experiments/run_direct_probing_wildchat.py
    # detach with Ctrl-B D, reattach later with: tmux attach -t wildchat-probe
...
PERSONAS_PATH = REPO_ROOT / "experiments" / "behavioral_audit" / "wildchat_personas.jsonl"
...
async def main() -> None:
    ...
    print(f"Subjects: {len(stage1_prompts)}  |  Classes: {COMBINED_CLASSES}")
    ...
    _stage1_log = REPO_ROOT / "logs" / f"{EXPERIMENT_NAME}-stage1"
