...
STAGE2 = {
    ...
    / f"{STAGE2_DIR}/direct-probing-combined-ministral3-8b-ministral-3-8b_modal-stage2.judgments.csv",
    "gemma-4-e2b_modal": REPO
    / f"{STAGE2_DIR}/direct-probing-combined-e2b-gemma-4-e2b_modal-stage2.judgments.csv",
}
...
for name, path in STAGE2.items():
    ...
    with open(out, "w", newline="") as f:
        ...
        w.writerows(rows)
    summary.append(
        {"model": name, "rows": len(rows), "masked_gender": masked_g, "masked_background": masked_b}
    )
    print(
        f"{name:24s} rows={len(rows):4d} masked_gender={masked_g:4d} masked_background={masked_b:4d} -> {out.name}"
    )
