...
Two persona dimensions present in each conversation history:
  ...
  Region:   Name   (e.g. "Michael Johnson") | Artist (e.g. "Linkin Park")
...
# Module-lifetime log handle: stays open across the whole script and is closed
# in the top-level `finally` block below, so a `with` block is not feasible here.
LOG = open(OUT / "run.log", "w", encoding="utf-8", buffering=1)  # noqa: SIM115
...
try:
    ...
    REGION_CSV = ROOT / "src" / "generate_backgrounds" / "dimension_value_mapping" / "region.csv"
    ...
    # ── Region indicators ───────────────────────────────────────────────────────
    region_phrase_to_slot: dict[str, str] = {}
    region_phrase_to_region: dict[str, str] = {}
    with open(REGION_CSV, encoding="utf-8-sig", newline="") as fh:
        for row in csv.DictReader(fh):
            p = row["Indicator_value"].strip()
            dim_col = "Dimension value" if "Dimension value" in row else "Dimension_value"
            region_phrase_to_slot[p] = row["Indicator_name"].strip()
            region_phrase_to_region[p] = row[dim_col].strip()

    name_phrases: set[str] = {p for p, s in region_phrase_to_slot.items() if s == "Name"}
    artist_phrases: set[str] = {p for p, s in region_phrase_to_slot.items() if s == "Artist"}
    log(f"Region Name   ({len(name_phrases)}): {sorted(name_phrases)[:4]}...")
    log(f"Region Artist ({len(artist_phrases)}): {sorted(artist_phrases)[:4]}...")

    # ── Placeholder map ───────────────────────────────────────────────────────
    phrase_to_placeholder: dict[str, str] = {}
    for p, slot in gender_phrase_to_slot.items():
        phrase_to_placeholder[p] = f"<{slot}>"
    for p, slot in region_phrase_to_slot.items():
        phrase_to_placeholder[p] = f"<{slot}>"

    all_phrases: set[str] = set(phrase_to_placeholder)

    PATS: dict[str, re.Pattern] = {
        p: re.compile(rf"(?<!\w){re.escape(p)}(?!\w)", re.IGNORECASE) for p in all_phrases
    }
    ...
    # Keep only histories with both a Gender turn and a Region turn.
    GENDER_MARKER = "I really enjoy watching "
    REGION_MARKER = "I really like the music from "
    valid_idx = [
        i
        for i, h in enumerate(histories)
        if any(GENDER_MARKER in m.get("content", "") for m in h)
        and any(REGION_MARKER in m.get("content", "") for m in h)
    ]
    ...
    # Balance genders.
    female_idx = [i for i, lbl in enumerate(labels) if lbl == "Female"]
    male_idx = [i for i, lbl in enumerate(labels) if lbl == "Male"]
    ...
    for p in sorted(gender_phrase_to_slot):
        ...
        if idxs:
            log(
                f"  {p!r:28s} ({gender_phrase_to_slot[p]}/{gender_phrase_to_gender[p]}) -> {len(idxs)}"
            )
                ...
                if m:
                    for p in _s_movie:
                        if PATS[p].search(m.group(1).strip()):
                            history_to_movie[i] = p
                            break
                h = HOBBY_SLOT_RE.search(c)
                if h:
                    for p in _s_hobby:
                        if PATS[p].search(h.group(1).strip()):
                            history_to_hobby[i] = p
                            break
            if REGION_MARKER in c:
                ...
                if a:
                    for p in _s_artists:
                        if PATS[p].search(a.group(1).strip()):
                            history_to_artist[i] = p
                            break

    # Slot-type assertions
    for _i, p in history_to_movie.items():
        assert gender_phrase_to_slot[p] == "Movie"
    for _i, p in history_to_hobby.items():
        assert gender_phrase_to_slot[p] == "Hobby"
    for _i, p in history_to_name.items():
        assert region_phrase_to_slot[p] == "Name"
    for _i, p in history_to_artist.items():
        assert region_phrase_to_slot[p] == "Artist"
    log(
        f"Slot detection: {len(history_to_movie)} movie | {len(history_to_hobby)} hobby | "
    ...
    texts_only_movie = [
        _format_messages(_ablate(h, hobby_phrases | name_phrases | artist_phrases))
        for h in histories
    ]
    texts_only_hobby = [
        _format_messages(_ablate(h, movie_phrases | name_phrases | artist_phrases))
        for h in histories
    ]
    texts_only_name = [
        _format_messages(_ablate(h, movie_phrases | hobby_phrases | artist_phrases))
        for h in histories
    ]
    texts_only_artist = [
        _format_messages(_ablate(h, movie_phrases | hobby_phrases | name_phrases))
        for h in histories
    ]
    ...
    if hs_path.exists():
        ...
        hs: dict[str, dict[int, np.ndarray]] = {
            c: {layer: _d[f"{c}_{layer}"] for layer in all_layers} for c in COND_NAMES
        }
        del _d
        log("Hidden states loaded")
    else:
        all_texts = (
            texts_full + texts_only_movie + texts_only_hobby + texts_only_name + texts_only_artist
        )
        ...
        hs = {
            c: {layer: _raw[layer][idx * N : (idx + 1) * N] for layer in all_layers}
            for idx, c in enumerate(COND_NAMES)
        }
        save_d = {f"{c}_{layer}": hs[c][layer] for c in COND_NAMES for layer in all_layers}
    ...
    def probe_cv(X: np.ndarray, y_arr: np.ndarray, n_splits: int = 5, seed: int = 42) -> float:
    ...
    def probe_generalization(
        X: np.ndarray, y_arr: np.ndarray, held_out_mask: np.ndarray, seed: int = 42
    ):
    ...
    log("Layer 0 (embedding only): " + " ".join(f"{c}={acc[c][0]:.3f}" for c in COND_NAMES))
    log("Probe training done")

    # Best layer per condition (used for generalization probes)
    best_layer_by_cond: dict[str, int] = {
        c: max(all_layers, key=lambda layer: acc[c][layer]) for c in COND_NAMES
    }
    ...
    def apply_probe(hs_dict: dict[int, np.ndarray]) -> np.ndarray:
        return clf_best.predict_proba(scaler_best.transform(hs_dict[best_layer]))[:, male_class]

    pm: dict[str, np.ndarray] = {c: apply_probe(hs[c]) for c in COND_NAMES}
    for c, pmv in pm.items():
        log(
            f"P(Male) [{c:12s}]: overall={pmv.mean():.3f}  "
    ...
    def compute_pm_oof(
        X: np.ndarray, y_arr: np.ndarray, n_splits: int = 5, seed: int = 42
    ) -> np.ndarray:
    ...
    pm_oof: dict[str, np.ndarray] = {
        c: compute_pm_oof(hs[c][best_layer_by_cond[c]], y) for c in COND_NAMES
    }
    ...
    def _pick_holdout(
        ...
        For Region indicators we compute the predominant gender from the dataset
    ...
    for cond, holdout_vals in HOLDOUT.items():
        ...
        held_out_mask = np.array(
            [slot_map.get(i, "") in holdout_vals for i in range(N)], dtype=bool
        )
    ...
    with open(OUT / "data" / "generalization_results.csv", "w", newline="", encoding="utf-8") as fh:
        w = csv.DictWriter(
            fh,
            fieldnames=[
                "condition",
    ...
    for c in COND_NAMES + ["ctrl"]:
        ax.plot(
            layers,
            [acc[c][layer] for layer in layers],
            markers_map[c],
            color=CCOLS[c],
            lw=1.5 if c == "ctrl" else 2,
            ms=5,
            label=CLABELS[c],
        )
    ...
    gen_conds = [
        c for c in ["only_movie", "only_hobby", "only_name", "only_artist"] if c in gen_results
    ]
    ...
    bars_std = ax.bar(
        ...
        std_accs,
        bar_w,
        color=[CCOLS[c] for c in gen_conds],
        alpha=0.9,
    ...
    bars_gen = ax.bar(
        ...
        alpha=0.45,
        hatch="///",
        label="Generalization accuracy (held-out values only)",
    )
            ...
            region_stats.append(
                {
                    "phrase": p,
                    "slot": slot,
                    "region_group": region_phrase_to_region[p],
    ...
    region_stats.sort(key=lambda r: r["pm_cond"])

    n_region_plot = len(region_stats)
    fig_h = max(12, max(len(gender_stats), n_region_plot) * 0.35 + 2)
    fig, (ax_l, ax_r) = plt.subplots(1, 2, figsize=(22, fig_h))

    def _dot_panel(ax, stats, title, _cond_key, x_label="pm_cond"):
        ...
        for i, row in enumerate(stats):
            ...
            base = (
                FCOL
                if row.get("gender_csv") == "Female" or row.get("gender_data") == "Female"
                else MCOL
                if row.get("gender_csv") == "Male" or row.get("gender_data") == "Male"
                else GREY
            )
        ...
        ax.legend(
            handles=[
                ...
                Line2D(
                    [0], [0], marker="D", color="grey", linewidth=0, ms=8, label="◆ ONLY this type"
                ),
    ...
    _dot_panel(
        ax_r,
        region_stats,
        "Region indicators: P(Male) in FULL vs ONLY_NAME / ONLY_ARTIST\n"
        "(● FULL with all 4  |  ◆ only this indicator type present)",
        "only_name",
    )
    ...
    for c in COND_NAMES:
        ...
        log(
            f"  [{c:12s}] best_layer={bl}  acc={acc[c][bl]:.3f}  "
    ...
    for cond, r in gen_results.items():
        log(
            f"  [{cond:12s}] std_acc={r['std_acc']:.3f}  "
