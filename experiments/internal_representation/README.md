...
pipeline excludes histories without a Gender label by default. Region can be
probed as an additional target using `--attributes Gender Region`, but it is not
enabled by default.
...
```bash
uv run python experiments/internal_representation/analyze_words.py --dataset-index 22
```
...
`src/generate_backgrounds/dimension_value_mapping/gender.csv`, adds ten
fixed-template control words, and removes each target from every history in
which it occurs. Multiword values are treated as complete phrases. The change
in the saved probe's `P(Male)` is aggregated across histories.

```bash
uv run python experiments/internal_representation/aggregate_word_analysis.py --batch-size 16
```
...
Paper figures use the committed Modal result folders under
`results_modal_gemma4_31b/`, `results_modal_gemma4_e2b/`,
`results_modal_ministral3_8b/`, plus sweep/ablation variants. Rebuild plots from
those directories or via [`../../finalresults.ipynb`](../../finalresults.ipynb).
...
uv run python experiments/internal_representation/download_model.py
```
...
A tested balanced run with 40 histories per Gender class (from the repo root):

```bash
uv run python experiments/internal_representation/main.py \
  --attributes Gender \
  --samples 40 \
  --device-map mps \
  --dtype float16 \
...
Install the Modal extra once, then sync the HF secret for gated Gemma repos
(accept the model licence on Hugging Face first):

```bash
uv sync --extra modal --extra internal-rep
...
```bash
MODAL_IR_GPU=L4 uv run modal run experiments/internal_representation/modal_run.py \
...
```bash
MODAL_IR_GPU=A100-80GB uv run modal run --detach experiments/internal_representation/modal_run.py \
  --model google/gemma-4-31B-it --samples 0 --dtype bfloat16 \
  --out results_modal_gemma4_31b
```

Default download dir is `experiments/internal_representation/results_modal/`
unless you pass `--out` (paper runs used `results_modal_gemma4_31b`,
`results_modal_gemma4_e2b`, `results_modal_ministral3_8b`). Local `results/`
runs are never clobbered. The large `hidden_states_personas.npz` intermediate is
not downloaded by default; set `MODAL_IR_KEEP_HIDDEN=1` to include it.
...
modal volume get pers-ir-results results_modal_gemma4_31b/ \
  experiments/internal_representation/results_modal_gemma4_31b
```
