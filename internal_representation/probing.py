...
def _train_and_eval_probe(
    ...
    cv = StratifiedKFold(n_splits=min(config.cv_folds, len(set(y_train))), shuffle=True, random_state=42)
...
def train_probes(
    ...
    for layer_idx in layers:
        ...
        y_train_ctrl = y_shuffled[: len(y_train)]
        y_test_ctrl = y_shuffled[len(y_train):]
