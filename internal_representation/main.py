...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Linear Probing Pipeline — Expertise")
    ...
    p.add_argument("--data-file", type=str, default=None,
                   help="Path to a pre-labeled JSON dataset (skips generation). "
                        "Schema: {conversations: [...], labels: {expertise: [...]}}")
...
def load_data_file(path: str) -> dict:
    ...
    with open(path) as f:
        data = json.load(f)
    assert "conversations" in data and "labels" in data and "expertise" in data["labels"], (
        f"--data-file must contain 'conversations' and 'labels.expertise' keys. Got: {list(data.keys())}"
    )
    return data
...
def main():
    ...
    print("=" * 70)
    print("  LINEAR PROBING PIPELINE — EXPERTISE")
    print("=" * 70)
    print(f"  Model:           {cfg.model.model_name}")
    print(f"  Device map:      {cfg.model.device_map}")
    print(f"  Dtype:           {cfg.model.torch_dtype}")
    print(f"  Samples/class:   {cfg.data.samples_per_class}")
    ...
    if args.data_file:
        ...
        dataset = load_data_file(args.data_file)
    else:
        dataset = prepare_dataset(cfg.data)

    conversations = dataset["conversations"]
    expertise_labels = dataset["labels"]["expertise"]
    print(f"  Total conversations: {len(conversations)}")
    print(f"  Class distribution: {dict(Counter(expertise_labels))}")
    ...
    le = LabelEncoder()
    le.fit(expertise_labels)

    probe_results, _ = train_probes(
        hidden_states, expertise_labels, "expertise", cfg.probe,
        test_size=cfg.data.test_size, seed=cfg.data.seed,
    )
    probe_results.save(os.path.join(cfg.results_dir, "probe_expertise.json"))
    ...
    plot_layer_accuracy(probe_results, "expertise", save_dir=cfg.plots_dir)
    plot_selectivity_gap(probe_results, "expertise", save_dir=cfg.plots_dir)

    best_real = probe_results.best_layer("expertise", "logistic", control=False)
    best_ctrl = probe_results.best_layer("expertise", "logistic", control=True)
    ...
    if best_real and best_ctrl:
        ...
        print(f"  expertise  layer={best_real.layer:3d}  "
