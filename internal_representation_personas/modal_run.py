...
WHY THIS EXISTS
    ...
    It reuses the same conventions as ``experiments/modal_gpu_poc/modal_serve.py``:
...
FULL RUN (the 31B dense model needs an 80 GB GPU)
    MODAL_IR_GPU=A100-80GB modal run internal_representation_personas/modal_run.py \\
        --model google/gemma-4-31B-it --samples 40 --dtype bfloat16

INDICATOR ABLATION (Gender-only; ablates gender + race indicators)
...
Artifacts land in ``internal_representation_personas/<out>/{results,plots,data}``
...
# Repo-relative paths (this file lives in internal_representation_personas/).
...
# Container paths.
_PIPELINE_DIR = "/app/pipeline"          # the bundled internal_representation_personas modules
_PERSONAS_REMOTE = "/app/personas.jsonl"  # bundled dataset (no repo checkout needed in-container)
# Indicator mapping CSVs bundled for the ablation stage (gender + race).
_GENDER_CSV_REMOTE = "/app/mappings/gender.csv"
_RACE_CSV_REMOTE = "/app/mappings/race.csv"
_ABLATION_MAPPINGS = [_GENDER_CSV_REMOTE, _RACE_CSV_REMOTE]
...
image = (
    ...
    # Indicator mappings for the ablation stage (Gender + Race).
    .add_local_file(str(_MAPPING_DIR_LOCAL / "gender.csv"), _GENDER_CSV_REMOTE)
    .add_local_file(str(_MAPPING_DIR_LOCAL / "race.csv"), _RACE_CSV_REMOTE)
)
...
def run_probe(
    ...
    Modes:
      ...
      * ``ablation`` — probe, then ablate gender+race indicators in the SAME
...
def sweep(
    ...
    """Run the per-condition layer-accuracy sweep for a prior probe run.
        ...
        MODAL_IR_GPU=A100-80GB modal run --detach \\
            internal_representation_personas/modal_run.py::sweep \\
...
def main(
    ...
    ``attributes`` is a space- or comma-separated list, e.g. "Gender Race".
    ``samples`` is the cap PER (gender, race) group; ``0`` (the default) means
    ...
    race indicators in one container. ``--ablation-only --from-run <name>``
    ...
    if ablation_only:
        ...
        scope = "ALL labelled histories" if samples_arg is None else f"SUBSAMPLE — {samples_arg} per (gender,race) group"
        print(f"[modal-run] sample scope: {scope}")
    if ablation or ablation_only:
        # ~25 gender + ~56 race indicators + 10 controls ≈ 91 phrases; ablation
        ...
        print(
            ...
            f"(gender+race). A full run (~1715/1715 × ~91) is hundreds of thousands of passes on a large "
