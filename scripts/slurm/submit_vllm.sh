...
#   bash scripts/slurm/submit_vllm.sh prod  helix SIF=$PWD/vllm-openai.sif
#
# Optional env:
...
case "$CLUSTER" in
  bwunicluster) SBATCH_FILE="scripts/slurm/bwunicluster.sbatch"; GRES="gpu:${TP}" ;;
  helix)        SBATCH_FILE="scripts/slurm/helix.sbatch";        GRES="gpu:A100:${TP}" ;;
esac
