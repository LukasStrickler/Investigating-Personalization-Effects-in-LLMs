...
# ── REGION-LEVEL PICKS ────────────────────────────────────────────────────────
region_picks = (
    tidy_project.groupby(["Name", "intended_gender", "perceived_region"])["respondent_id"]
    .nunique()
    .reset_index()
...
dominant_gender = (
    tidy_project.drop_duplicates(["respondent_id", "Name", "perceived_region", "perceived_gender"])
...
dominant_gender = (
    dominant_gender.sort_values(["gender_count", "gender_rank"], ascending=[False, True])
...
source_country_counts = (
    tidy_project.groupby(["Name", "perceived_region", "perceived_country"])["respondent_id"]
    .nunique()
    .reset_index()
...
source_country_str = (
    source_country_counts.sort_values("country_count", ascending=False)
    .groupby(["Name", "perceived_region"])
    .apply(
        lambda x: ", ".join(
            f"{row['perceived_country']} ({row['country_count']})" for _, row in x.iterrows()
        ),
...
base["origin_match"] = (base["intended_region"] == base["perceived_region"]).astype(int)
...
def top_with_unique_surnames(
    ...
    not_fit: list[pd.Series] = []

    ranked = group.sort_values(
        ["adjusted_perception_pct", "category_selected_count"], ascending=[False, False]
    )
...
top_rows = []
for _region, group in base.groupby("perceived_region"):
    shared_surnames: set[str] = set()
    shared_firstnames: set[str] = set()

    for gender in ["Male", "Female"]:
        top = top_with_unique_surnames(
            group, gender, used_surnames=shared_surnames, used_firstnames=shared_firstnames, n=3
        )
...
top_names = top_names.drop_duplicates(subset=["perceived_region", "selected_gender_group", "Name"])
...
top_names = (
    ...
    .sort_values(
        ["perceived_region", "selected_gender_group", "rank"], ascending=[True, True, True]
    )
    .reset_index(drop=True)
)
