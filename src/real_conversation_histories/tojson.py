...
def to_persona_gender(label: str | None, strict: bool) -> str | None:
    return (_STRICT if strict else _LOOSE).get((label or "").strip())


def combination_id(gender_value: str | None) -> str | None:
...
def is_kept(cell: object, keep_value: str) -> bool:
...
def load_keep_map(
    ...
    with open(checked_csv, encoding="utf-8-sig", newline="") as fh:
        ...
        if reader.fieldnames is None:
            sys.exit(f"ERROR: {checked_csv} is empty (no header row).")
        for col in (correct_col, id_col, gender_col):
            if col not in reader.fieldnames:
                sys.exit(
                    f"ERROR: column '{col}' not found in {checked_csv}.\n"
...
def wildchat_conversation_id(example: dict[str, Any]) -> str | None:
...
def clean_messages(conversation: list[Any] | None) -> list[dict[str, Any]]:
...
def flatten_transcript(messages: list[dict[str, Any]], max_chars: int) -> str:
...
def main() -> None:
    ...
    ap.add_argument(
        "--checked-csv",
        default=OUTPUT_DIR / "wildchat_gender_evidence_results_checked.csv",
        help="The hand-checked CSV (with the 'correct' column).",
    )
    ap.add_argument(
        "--out",
        default=OUTPUT_DIR / "wildchat_personas.jsonl",
        help="Output JSONL path (personas schema, full conversations).",
    )
    ap.add_argument(
        "--csv",
        default=OUTPUT_DIR / "wildchat_personas.csv",
    ...
    ap.add_argument(
        "--conversations",
        default=OUTPUT_DIR / "wildchat_conversations.jsonl",
        help="Local JSONL of full conversations from conversation_histories_extraction.py "
    ...
    ap.add_argument("--keep-value", default="1", help="Cell value that means keep (default: '1').")
    ...
    print(
        f"Checked CSV: {stats['total_rows']:,} rows | "
    ...
    with contextlib.ExitStack() as stack:
        ...
        csv_fh = stack.enter_context(open(args.csv, "w", encoding="utf-8", newline=""))
        csv_writer = csv.DictWriter(csv_fh, fieldnames=csv_fields)
        csv_writer.writeheader()

        def emit(history_id: str, gender: str, messages: list[dict[str, Any]]) -> None:
        ...
        if used_local:
            ...
            print(
                f"Local conversations file '{args.conversations}' not found — "
    ...
    if pending:
        print(
            f"WARNING: {len(pending):,} kept ids were NOT found"
